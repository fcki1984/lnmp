...
DB_Info=('MySQL 5.1.73' 'MySQL 5.5.62' 'MySQL 5.6.51' 'MySQL 5.7.44' 'MySQL 8.0.39' 'MySQL 8.4.7' 'MariaDB 5.5.68' 'MariaDB 10.4.34' 'MariaDB 10.5.26' 'MariaDB 10.6.19' 'MariaDB 10.11.9')
PHP_Info=('PHP 5.2.17' 'PHP 5.3.29' 'PHP 5.4.45' 'PHP 5.5.38' 'PHP 5.6.40' 'PHP 7.0.33' 'PHP 7.1.33' 'PHP 7.2.34' 'PHP 7.3.33' 'PHP 7.4.33' 'PHP 8.0.30' 'PHP 8.1.29' 'PHP 8.2.23' 'PHP 8.3.11' 'PHP 8.4.16')
...
Verify_Tarball()
{
    ...
    echo "Warning: ${FileName} failed integrity check, re-downloading..."
    rm -f "${FileName}"
    if [ -n "${URL}" ]; then
    ...
    if ! tar tzf "${FileName}" >/dev/null 2>&1; then
        Echo_Red "Integrity verification failed for ${FileName}, aborting to avoid broken install."
        exit 1
    fi
}
